...
CREATE OR REPLACE VIEW global_buys AS
    SELECT *
    FROM dblink('dbname=amazoon host=127.0.0.1 user=postgres password=postgres',
...
CREATE OR REPLACE VIEW global_products AS
    SELECT *
    FROM
...
CREATE OR REPLACE VIEW global_worker AS 
    SELECT *
...
CREATE OR REPLACE VIEW global_sells AS 
    SELECT * 
    FROM dblink('dbname=manolo host=127.0.0.1 user=postgres password=postgres',
...
CREATE OR REPLACE VIEW global_stores AS
    SELECT *
    FROM
    dblink('dbname=amazoon host=127.0.0.1 user=postgres password=postgres',
